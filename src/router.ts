...
export type PathsWithParams =
	...
	| '/u/:uid/compose'
	| '/u/:uid/search';
export type PathsWithoutParams = '/' | '/register' | '/login';
...
export const A = <P extends Paths>(props: AnchorWithParamProps<P>) => {
	...
	return A_(
		mergeProps(ext, {
			get href() {
				return int.params ? generatePath(int.href, int.params) : int.href;
			},
		})
	);
};
export const Navigate = <P extends Paths>(props: NavigateWithParamProps<P>) => {
	return Navigate_(
		mergeProps(props, {
			get href() {
				return props.params ? generatePath(props.href, props.params) : props.href;
			},
		})
	);
};
