/**
 * @fileoverview
 * errorstacks, by marvinhagemeister
 * https://github.com/marvinhagemeister/errorstacks/blob/e2df682e48def6bdc19e06acd3e7dd864044a4f1/src/index.ts
 *
 * Vendored copy because the published package isn't done correctly, causing
 * TypeScript to not properly resolve the types
 */
...
const createRawFrame = (raw: string): StackFrame => {
	return {
		column: -1,
		fileName: '',
		line: -1,
...
const parseWebkit = (str: string): StackFrame => {
...
const parseChromeIe = (lines: string[]): StackFrame[] => {
